...
class SetPath extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            name: '',
    ...
    submitHandler = () => {
        ...
        let obj = {
            Template: this.state.name
        }
        
        const isValid = this.isValid(this.state.name);

        if(isValid === true){
            addToParent('SetPath', obj);
    ...
    render(){
        ...
        return (
            <Modal open={modalOpen} size='mini'>
                <Header icon='save' content='SetPath Filter' />
                <Modal.Content>
...
export default connect(mapStateToProps, mapDispatchToProps)(SetPath)