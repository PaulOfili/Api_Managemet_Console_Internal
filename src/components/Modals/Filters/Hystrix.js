...
class Hystrix extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            name: '',
    ...
    isValid = (name) => {
        ...
        if(validator.isAlpha(name) === false)
            return 'Field must contain only string'
        else
            return true
    }
    ...
    submitHandler = () => {
        ...
        let obj = {
            Hystrix: this.state.name
        }
        
        const isValid = this.isValid(this.state.name);

        if(isValid === true){
            addToParent('Hystrix', obj);
    ...
    render(){
        ...
        return (
            <Modal open={modalOpen} size='mini'>
                <Header icon='save' content='Hystrix Filter' />
                <Modal.Content>
...
export default connect(mapStateToProps, mapDispatchToProps)(Hystrix)