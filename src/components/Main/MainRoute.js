...
import Dropdown from '../Routes/common/Dropdowns';
import Table from '../Routes/common/Table';
import { predicateFields, filterFields } from '../Routes/fields';
import { showModal } from '../Modals/actions/index';
import { filterOptions, predicateOptions } from '../Routes/routesConstants';
import { setRoute } from '../Routes/actions/routesActions';
import { updateRoute, getRoute } from '../Routes/actions/routesThunks';
import Access from '../common/Access';
import validator from 'validator';


function MainRoute({match}) {
    ...
    const generateFilter = useCallback((array) => {
        ...
        for(let i=0; i<array.length; i++){
            if(!sieve[array[i].name]){
                ...
                let filterObj = filterFields[array[i].name.toLowerCase()]
    ...
    useEffect(() => {
        getCurrentRoute(match.params.id);
    }, [match, getCurrentRoute])       
    ...
    useEffect(() => {
        if(route && route.id){
            ...
            setFormValues(prevState => ({ ...prevState, routeName: match.params.id}));
        }
    }, [route, generateFilter, generatePredicate, match])
    ...
    const handlePredicateSelect = (e, { value }) => {
        let prevState = formState.predicatesTracker;
        let received = value;
        if( prevState.length < received.length){
            ...
            let item = findMissingItem(prevState, received);
    ...
    const handleFilterSelect = (e, { value }) => {
        let prevState = formState.filtersTracker;
        let received = value;
        if( prevState.length < received.length){
            const lastItem = value[value.length - 1].toLowerCase()
    ...
    const parser = (array) => {
        ...
        for(let i=0; i<array.length; i++){
          ...
          array[i].rows.map( (item, index) => index === array[i].rows.length -1 ? string += `${Object.values(item)}` : string += `${Object.values(item)}, `)
          results.push(string);
        }
        return results;
      }
